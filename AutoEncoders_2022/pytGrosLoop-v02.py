...
resumeName = data_res + '/resume_' + '{:d}'.format(t.year) + '{:02d}'.format(t.month) + '{:02d}'.format(t.day) + '-'  + '{:02d}'.format(t.hour) + '{:02d}'.format(t.minute) + '.html'
print(resumeName)
fResume = open(resumeName, 'w')  # web page
fResume.write("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 3.2 Final//EN\">\n")
fResume.write("<html>\n")
fResume.write("<head>\n")
fResume.write("<meta http-equiv=\"content-type\" content=\"text/html; charset=UTF-8\" />\n")
fResume.write("<title> Resume of ZEE_14 predictions </title>\n")  # 
fResume.write("</head>\n")

fResume.write(' <h1><center><b><font color=\'blue\'>AutoEncoder with {:d} files.</font></b></center></h1> <br>\n'.format(nbFiles))
fResume.write('<table border=\"1\" cellpadding=\"5\" width=\"50%\">\n')
fResume.write('<tr>\n')
fResume.write('<td>\n')
fResume.write('nbFiles = ' + str(nbFiles) + '<br>\n')
fResume.write('there is {:03d} datasets<br>\n'.format(nbBranches))
fResume.write('device : {:s}<br>\n'.format(str(device)))
fResume.write('</td>\n')
fResume.write('<td>\n')
fResume.write('learning rate lr = ' + str(lr) + '<br>\n')
fResume.write('epsilon = ' + str(epsilon) + '<br>\n')
fResume.write('</td>\n')
fResume.write('<td>\n')
fResume.write('hidden_size_1 = ' + str(hidden_size_1) + '<br>\n')
fResume.write('hidden_size_2 = ' + str(hidden_size_2) + '<br>\n')
if useHL3 == 1:
    fResume.write('hidden_size_3 = ' + str(hidden_size_3) + '<br>\n')
if useHL4 == 1:
    fResume.write('hidden_size_4 = ' + str(hidden_size_4) + '<br>\n')
fResume.write('latent_size = ' + str(latent_size) + '<br>\n')
fResume.write('</td>\n')
fResume.write('<td>\n')
fResume.write('batch_size = ' + str(batch_size) + '<br>\n')
fResume.write('nb_epochs = ' + str(nb_epochs) + '<br>\n')
fResume.write('percentageTrain = ' + str(percentageTrain) + '<br>\n')
fResume.write('</td>\n')
fResume.write('</tr>\n')
fResume.write('</table>\n')
fResume.write('<br>\n')
...
fResume.write('<table border=\"1\" cellpadding=\"5\" width=\"100%\">\n')
NTitles = len(histoKeysNames)
for i in range(0,NTitles): # NTitles
    short_histo_name = reduceBranch(histoKeysNames[i][0])
    if ( i % 5  == 0 ):
        fResume.write("\n<tr valign=\"top\">\n")
    fResume.write("\n<td width=\"10\">\n<b> " + histoKeysNames[i][0] + "</b>"+ "<br><br>")

    histoPrevious = ""
    numLine = 0
    for elem in histoKeysNames[i][1]:
        #print('\n=== histo :  %s'%elem[0])
        short_histo_name = reduceBranch(elem[0])
        after, before, common = testExtension(short_histo_name, histoPrevious)
        #print('after : {}, before : {}, common : {}'.format(after, before, common))

        histo_position_1 = elem[1]
        #print('histo_position : ', histo_position_1, ' - numline : ', numLine)
        if numLine == 0:
            fResume.write("<br>")
        if ( histo_position_1 == "0" ):
            if (numLine == 0):
                fResume.write("<a href=\"#" +short_histo_name + "\"><font color=green>" +short_histo_name + "</font></a>" + "&nbsp;\n")
                common = short_histo_name
                numLine += 1
            else: # numLine > 0
                if ( after == "" ):
                    fResume.write("<a href=\"#" +short_histo_name + "\"><font color=green>" +before + "</font></a>" + "&nbsp;\n")
                else: # $after != ""
                    fResume.write("<a href=\"#" +short_histo_name + "\"><font color=green>" +after + "</font></a>" + "&nbsp;\n")
                common = before
        else: #histo_position_1 == "1"
            if (numLine == 0):
                fResume.write("<a href=\"#" + short_histo_name + "\"><font color=grey>" + short_histo_name + "</font></a>" + "&nbsp;\n") # "<br>\n";
                common = short_histo_name
            else: # numLine > 0
                if ( after == "" ):
                    fResume.write("<a href=\"#" + short_histo_name + "\"><font color=blue>" + before + "</font></a>" + "&nbsp;\n") # "<br>\n";
                else: # $after != ""
                    fResume.write("<a href=\"#" + short_histo_name + "\"><font color=blue>" + after + "</font></a>" + "&nbsp;\n") # "<br>\n")
            numLine = 0
...
fResume.write('<table border=\"0\" cellpadding=\"5\" width=\"100%\">\n')
...
for i in range(0, loopMaxValue):
    ...
    fResume.write('<tr valign=\"top\">\n')
    fResume.write('<td><a href=\"#\"><img width=\"18\" height=\"18\" border=\"0\" align=\"middle\" src=' + image_up + ' alt=\"Top\"/></a></td>\n')
    fResume.write('<td>\n')
    fResume.write('<a id=\"' + short_histo_name + '\">' + '\n')
    fResume.write(' <font color=\'blue\'>{:s}</font> <br>\n'.format(str(branches[i])))
    fResume.write('</a>' + '\n')
    ...
    if ( useEncoder == 1):
        ...
        fResume.write('coefficient losses : {:1.4e}'.format(rr) + ' - {:1.4e}<br>\n'.format(ss))
        if (max(ss, rr) <= 10.):
            print(colorText('losses curves seems OK.', 'green'))
            fResume.write('<font color=\'green\'>losses curves seems OK.</font><br>\n')
            fHisto.write('<font color=\'green\'>losses curves seems OK.</font><br>\n')
        elif (max(ss,rr) <= 25):
            print(colorText('losses curves could be wrong.', 'blue'))
            fResume.write('<font color=\'blue\'>losses curves could be wrong.</font><br>\n')
            fHisto.write('<font color=\'blue\'>losses curves could be wrong.</font><br>\n')
        else: # great value
            print(colorText('losses curves is certainly wrong.', 'red'))
            fResume.write('<font color=\'red\'>losses curves is certainly wrong.</font><br>\n')
            fHisto.write('<font color=\'red\'>losses curves is certainly wrong.</font><br>\n')
        fResume.write('<br>\n')
    ...
    predLossesValues = folderName + "/predLossesValues_" + branches[i] + "_multi.txt"
    ...
    fHisto.write("\n</table>\n")
    fHisto.write("</td>")
    pictureName = folderName + '/comparison_loss_values_' + branches[i] + '_' + rel[6:] + '.png'
    ...
    #fResume.write('<td><a href=\"' + pictureName + '\"><img width=\"450\" height=\"250\" border=\"0\" align=\"middle\" src=\"' + pictureName + '\"></a></td>')
    fResume.write('<a href=\"' + histoPath + '\"><img width=\"450\" height=\"250\" border=\"0\" align=\"middle\" src=\"' + pictureName + '\"></a>')
