...
branches = ['h_ele_vertexPt', 'h_recCoreNum', 'h_recEleNum', # 0 1 2 
            ...
            'h_ele_sigmaIetaIetaVsPt_pfx', 'h_scl_EoEtrueVsrecOfflineVertices_pfx'] # 8 9
print(branches)
...
lossesValues = data_res + "/Losses/lossesValues_" + branch + ".txt"
...
#load new/old data from branchesHistos_RelRef.txt file ..
...
if (useTrainLoader == 1):
    ...
    '''if '_pfx' in branch:
        print('pfx')
        torch_tensor_entries_n = torch_tensor_entries
    else:
...
folderName += "_LT.{:02d}".format(latent_size) + '/' + "{:04d}".format(nbFiles) + '/' + branch + '/'
print('\n'+folderName)
...
for elem in linOp:
    ...
    pictureName = folderName + '/predicted_new_curves_' + branch + '_' + rel[6:] + '.png'
    #creatPredPictLinLog(branch, Ncols, torch_tensor_entries_n, y_pred_new, new_loss, rel[6:], pictureName)
    creatPredPictLinLog(branch, Ncols, torch_tensor_entries_n, torch_tensor_entries_n, new_loss, rel[6:], pictureName)
...
pictureName = folderName + '/comparison_loss_values_' + branch + '_' + rel[6:] + '.png'
createCompLossesPicture(labels,val,branch, pictureName)
