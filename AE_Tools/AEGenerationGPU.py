...
#    print("AEGeneration - arg. 4 :", sys.argv[4]) # dataset name
    print("AEGeneration - arg. 3 :", sys.argv[3]) # cpu/gpu option
    print("AEGeneration - arg. 4 :", sys.argv[4]) # timeFolder
...
pathAE = pathNb_files + '/AE_RESULTS_GPU/'
...
for elem in sortedArrayKSValues:
    ...
    l_tmp1.append(elem[0]) # give the histo name

l_tmp1 = list(set(l_tmp1)) # keep only on histo name
for elem in l_tmp1:
    print("histo name : {:s}".format(elem))
print('len l_tmp1 : {:d}'.format(len(l_tmp1)))
...
for i in range (0, N_histos):
    ...
    if Path(fileName).exists():
        print('\n{:d}/{:d} - {:s} exist'.format(i, N_histos - 1, name))
        tmp_branches.append(branches[i])
    else:
        print('\n{:d}/{:d} - {:s} does not exist'.format(i, N_histos - 1, fileName))
N_histos = len(tmp_branches)
print('N_histos : {:d}'.format(N_histos))
branches = tmp_branches

if (len(l_tmp1) < N_histos):
    branches = l_tmp1
    N_histos = len(l_tmp1)
print('N_histos : {:d}'.format(N_histos))
#stop()
...
for i in range(0, N_histos):#, N_histos-1 range(N_histos - 1, N_histos):  # 1 N_histos histo for debug
    branch = branches[i]
    print('histo : {:s}'.format(branch)) # print histo name
    
    #histo_rel = h_rel.Get(branch)
    #if (histo_rel):
    print('\n%s OK' % branch)
    name = pathNb_evts + "histo_" + branch + '_{:03d}'.format(nbFiles) + ".txt"
    ...
    #if i < 19:#'_pfx' in branch:
        #print('{:s} is pfx, continue.'.format(branch))
    #    continue
    ...
    if ( useEncoder == 1):
        ...
        for epoch in range(nb_epochs):
            ...
            encoded_out = encoded_out.cpu()
            test_loss = test_loss.cpu()
            #d_out = d_out.cpu() # not used
    ...
    for elem in linOp:
        ...
        new_loss, y_pred_new, latent_out = test_epoch_den(encoder=encoder,
                ...
                loss_fn=loss_fn)
        new_loss = new_loss.cpu()
        y_pred_new = y_pred_new.cpu()
        latent_out = latent_out.cpu()
        ...
        creatPredPictLinLog(branch, Ncols, torch_tensor_entries_n.cpu(), y_pred_new, new_loss, rel[6:], pictureName)
