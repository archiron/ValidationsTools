...
JobName="compAELosses_serial_job_test" # for slurm
output="/sps/cms/chiron/TEMP/compAELosses_%j.log" # for slurm
...
if [[ "$Choice" == "LLR" ]] 
  ...
  then
    ...
    sbatch -L sps -n 4 --mem=16000 -t 0-1:0:0 -J $JobName -o $output compAELosses.sh $LOG_AE_SOURCE $COMMON_SOURCE $FileName 
    deactivate
fi
