  ...
  then
    ...
    do
      if [[ $line == *"ElectronMcSignalValidator/"* ]]; then
        ...
        arrLine=(${line//dator/ })
        echo "${arrLine[1]}"
        #/opt/exp_soft/cms/t3/t3submit -8c -long -name "KS_stats_1000 v7b" checkDiffTest_2.sh $LOG_SOURCE $LOG_KS_SOURCE $COMMON_SOURCE $FileName "b" ${arrLine[1]}
        . checkDiffTest_2.sh $LOG_SOURCE $LOG_KS_SOURCE $COMMON_SOURCE $FileName "i" ${arrLine[1]} &
        #let "var++"
        #if [ $var == 10 ]
        #then
