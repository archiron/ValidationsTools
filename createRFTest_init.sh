...
if [[ "$Choice" == "LLR" ]] 
  then
    ...
    do
      echo "==> $i"
      /opt/exp_soft/cms/t3/t3submit -8c -long -name "CMSSW_15_0_0_pre3" createRFTest.sh $i $LOG_SOURCE $NB_EVTS $RESULTFOLDER $initialSEED
      #. createRFTest.sh $i $LOG_SOURCE $NB_EVTS $RESULTFOLDER $initialSEED #&
    done
fi
